...
exports.sendExamCountdownEmails = exports.COUNTDOWN_SEQUENCE = void 0;
...
function shell(inner, ctaLabel, footer = "") {
...
<p>Your dashboard already knows your weak domains. Start there.</p>`, "Run a readiness check", footer),
    },
    {
        daysOut: 7,
        stampKey: "d7",
...
<p>One honest mock plus a careful review is worth more than three rushed ones.</p>`, "Start a full-length mock", footer),
    },
    {
        daysOut: 3,
        stampKey: "d3",
...
<p>Tired and over-prepared loses to rested and prepared. Taper.</p>`, "Do a light review set", footer),
    },
    {
        daysOut: 1,
        stampKey: "d1",
...
<p>You've put in the reps. Trust the preparation — it's in there.</p>`, "Skim your exam-day checklist", footer),
    },
];
/** UTC-midnight boundary `daysFromToday` days after today. */
function utcDayStart(daysFromToday) {
    const d = new Date();
 ...
 */
exports.sendExamCountdownEmails = functions.pubsub
    ...
    if (!apiKey) {
        console.warn("[countdown] RESEND_API_KEY not set — skipping send (deploy-safe no-op). " +
    ...
    let skipped = 0;
    let failed = 0;
    for (const mail of exports.COUNTDOWN_SEQUENCE) {
        ...
        for (const planDoc of plansSnap.docs) {
            ...
            try {
                ...
                const message = axios_1.default.isAxiosError(err)
                    ? JSON.stringify((_b = (_a = err.response) === null || _a === void 0 ? void 0 : _a.data) !== null && _b !== void 0 ? _b : err.message)
                    : String(err);
                console.error(`[countdown] D-${mail.daysOut} send failed for plan ${planDoc.id} (user ${userId}): ${message}`);
                failed++;
            ...
            try {
                await planDoc.ref.set({
                    ...
                    [`countdownEmailsSentAt.${mail.stampKey}`]: admin.firestore.FieldValue.serverTimestamp(),
                }, { merge: true });
            }
            catch (err) {
                console.error(`[countdown] sent D-${mail.daysOut} for plan ${planDoc.id} but failed to stamp: ${String(err)}`);
            }
            sent++;
        }
    }
...
//# sourceMappingURL=examCountdown.js.map