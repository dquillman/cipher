...
async function checkOne(doc) {
    ...
    let res;
    try {
        res = await axios_1.default.get(s.url, {
            headers: {
        ...
        return Object.assign(Object.assign({}, base), { status: neverSucceeded ? "never_fetched" : "unreachable", error: err.message, changed: false, neverSucceeded });
    }
    if (res.status !== 200) {
        ...
        return Object.assign(Object.assign({}, base), { status: neverSucceeded ? "never_fetched" : "unreachable", httpStatus: res.status, changed: false, neverSucceeded });
    }
    ...
    const status = changed
        ...
        : s.status === "reviewed_ok"
            ? "reviewed_ok"
            : "ok";
    await doc.ref.update(Object.assign(Object.assign({ lastCheckedAt: admin.firestore.Timestamp.now(), lastSuccessAt: admin.firestore.Timestamp.now(), status, lastKnownSignature: signature, lastKnownExcerpt: excerpt, lastKnownBytes: bytes }, (changed
        ? {
            lastChangeDetectedAt: admin.firestore.Timestamp.now(),
            previousExcerpt: s.lastKnownExcerpt || null,
            previousBytes: (_a = s.lastKnownBytes) !== null && _a !== void 0 ? _a : null,
        }
        : {})), { lastErrorCode: null, lastErrorMessage: null, consecutiveFailures: 0 }));
    return Object.assign(Object.assign({}, base), { status, changed, neverSucceeded: false, bytes, previousBytes: s.lastKnownBytes });
}
function buildAlert(changed, broken) {
    const parts = [];
    if (changed.length) {
        parts.push(`<h2 style="margin:0 0 8px;font-size:18px">Outline changed — ${changed.length} source${changed.length > 1 ? "s" : ""}</h2>`, `<p style="margin:0 0 16px;color:#334155">A certifying body has published a change. Until someone confirms what moved, assume the matching question bank is stale.</p>`, "<ul style='padding-left:18px;color:#0f172a'>", ...changed.map((c) => {
            var _a, _b;
            return `<li style="margin-bottom:10px"><strong>${c.examName || c.name}</strong><br>` +
                `<a href="${c.url}" style="color:#4f46e5">${c.url}</a><br>` +
                `<span style="color:#64748b;font-size:13px">content size ${(_a = c.previousBytes) !== null && _a !== void 0 ? _a : "?"} → ${(_b = c.bytes) !== null && _b !== void 0 ? _b : "?"}${c.examId ? ` · bank <code>${c.examId}</code>` : ""}</span></li>`;
        }), "</ul>");
    }
    if (broken.length) {
        parts.push(`<h2 style="margin:20px 0 8px;font-size:18px">Watcher blind — ${broken.length} source${broken.length > 1 ? "s" : ""}</h2>`, `<p style="margin:0 0 16px;color:#334155">These cannot be fetched, so they are <em>not</em> being monitored. A source that has never succeeded has been silently blind since it was added.</p>`, "<ul style='padding-left:18px;color:#0f172a'>", ...broken.map((b) => `<li style="margin-bottom:10px"><strong>${b.examName || b.name}</strong>` +
            ...
            `<span style="color:#64748b;font-size:13px">${b.httpStatus ? `HTTP ${b.httpStatus}` : b.error || "unreachable"}</span></li>`), "</ul>");
    }
...
const performExamUpdateCheck = async () => {
    ...
    console.log(`[examWatch] ${outcomes.length} checked · ${changed.length} changed · ${broken.length} unreachable ` +
    ...
    if (!apiKey) {
        ...
        const msg = `[examWatch] ${changed.length} change(s) and ${broken.length} unreachable source(s) detected ` +
...
//# sourceMappingURL=examWatch.js.map