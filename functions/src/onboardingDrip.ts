...
export const scheduleOnboardingDrip = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    if (!apiKey) {
      ...
      console.error(
        `[drip] RESEND_API_KEY not set — NO ONBOARDING EMAIL SENT for ${uid}. ` +
      ...
      await snap.ref
        .set(
          { onboardingDripBlockedAt: admin.firestore.FieldValue.serverTimestamp() },
          { merge: true },
        )
        .catch(() => undefined);
      return;
    ...
    if (!compliance.ok) {
      await recordComplianceBlock("scheduleOnboardingDrip", compliance.missing);
      await snap.ref
        .set(
          { onboardingDripBlockedAt: admin.firestore.FieldValue.serverTimestamp() },
          { merge: true },
