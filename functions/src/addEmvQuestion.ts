...
/**
 * Bloom's Taxonomy is a first-class field on every question.
 * New questions MUST include bloomLevel. If you forget, the TS compiler will
 * error; if you bypass TS, run the backfill pipeline in /functions:
 *   node export-blooms-input.js     # exports un-classified questions
 *   node classify-blooms.js         # classifies via OpenAI
 *   node writeback-blooms.js --commit   # writes bloomLevel to Firestore
 */
type BloomLevel =
  | "Remember" | "Understand" | "Apply"
  | "Analyze" | "Evaluate" | "Create";

interface EmvQuestion {
  id: string;
  type: "emv";
  examId: string;
  domain: string;
...
const EMV_QUESTIONS: EmvQuestion[] = [
  {
    id: "emv-sample-001",
    type: "emv",
