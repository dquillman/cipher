...
export function countAnswered(data: { answers?: unknown } | undefined | null): number {
  const answers = data?.answers;
  if (!Array.isArray(answers)) return 0;
  return answers.filter((a) => a && (a as { selectedOption?: unknown }).selectedOption !== undefined).length;
}
 ...
 * un-gameable.
 */
export function answeredDelta(
  before: { answers?: unknown } | undefined | null,
  after: { answers?: unknown } | undefined | null,
): number {
...
export function usageCounterId(uid: string, day: string): string {
  return `${uid}_${day}`;
}

export const trackAnswerUsage = functions.firestore
 ...
 */
export async function getAnsweredToday(uid: string, now: Date = new Date()): Promise<number> {
  const day = usageDayKey(now);
  const snap = await admin.firestore().doc(`usageCounters/${usageCounterId(uid, day)}`).get();
  return snap.exists ? Number(snap.data()?.answeredCount || 0) : 0;
}
