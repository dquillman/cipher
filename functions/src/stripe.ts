...
import { getCheckoutUrls, getSubscriptionPrices, parseBillingInterval } from "./billingConfig";
...
export const createCheckoutSession = functions.https.onCall(async (data, context) => {
    ...
    const billingInterval = parseBillingInterval(data?.billingInterval);
    if (!billingInterval) {
        ...
        if (existingCustomerId) {
            ...
            if (live.data.length > 0 || trialing.data.length > 0) {
                throw new functions.https.HttpsError(
                    'failed-precondition',
                    'You already have an active subscription. Manage it from your Account page rather than buying a second one.',
                );
            }
        }
...
export const createPortalSession = functions.https.onCall(async (_data, context) => {
    ...
    try {
        ...
        const session = await stripe.billingPortal.sessions.create({
            customer: customerId,
            return_url: getCheckoutUrls().portalReturnUrl,
        });
    ...
    try {
        switch (event.type) {
            ...
            case 'customer.subscription.updated':
                ...
                await syncSubscriptionState(event.data.object as Stripe.Subscription | Stripe.Invoice);
                break;
            default:
...
async function handleCheckoutSessionCompleted(
    ...
    const allowedPriceIds = Object.values(getSubscriptionPrices());
    ...
    if (session.metadata?.expectedPriceId !== actualPriceId) {
 ...
 */
async function syncSubscriptionState(object: Stripe.Subscription | Stripe.Invoice) {
    const db = admin.firestore();
    ...
    await usersSnap.docs[0].ref.set({
        ...
        ...(status === 'canceled' ? { isPro: false, plan: 'starter', access: 'free', accessLevel: 'free' } : {}),
        lastUpdated: admin.firestore.FieldValue.serverTimestamp(),
    }, { merge: true });
