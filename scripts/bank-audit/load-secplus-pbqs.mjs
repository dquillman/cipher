/**
 * Validates and loads the Security+ PBQ set.
 *
 *   node load-secplus-pbqs.mjs             validate + dry run, writes nothing
 *   node load-secplus-pbqs.mjs --apply     insert
 *   node load-secplus-pbqs.mjs --undo --apply   delete everything this inserted
 *
...
const seed = JSON.parse(readFileSync('./seed/security-plus-pbqs.json', 'utf8'));
...
if (errs.length) {
    ...
    console.error('\n  Nothing written. Fix build-secplus-pbqs.mjs and re-run.');
    process.exit(1);
}
const counts = {};
